...
int solve() {
	// **** GET SUFFIX ARRAY **** //
	ord = sa = vi(n+1);
	FOR(i, 0, n) ord[i] = s[i], sa[i] = i; ord[n] = 0;
	for(int t = 1; t <= n; t *= 2) { // t <= n까지 돌리지 않으면 n = 1인 경우 ord[i] = str[i]가 되므로, ord[i]를 rev로 써먹을 수 없다!
		// radix sort = 2자리수 counting sort
		int sz = max(257, n+1); // ord 초기화 시 s[i]를 그대로 넣었기 때문에 257과 max
		// (i+t)를 보고 counting sort
		vi cnt, tmp;
		cnt = tmp = vi(sz, 0);
		FOR(i, 0, n) ++cnt[ord[min(n, i+t)]];
		FOR(i, 1, sz) cnt[i] += cnt[i-1];
		FOR(i, 0, n) tmp[--cnt[ord[min(n, i+t)]]] = i; // sa[i] = i번째 suffix (이 부분 헷갈리는거 주의)
		// (i)를 보고 counting sort (이 때, i+t 결과를 유지하기 위해 stable sort를 해야 함.)
		cnt = vi(sz, 0);
		FOR(i, 0, n) ++cnt[ord[i]];
		FOR(i, 1, sz) cnt[i] += cnt[i-1];
		RFOR(i, n-1, 0) sa[--cnt[ord[tmp[i]]]] = tmp[i]; // 여기서는 stable sort (거꾸로 loop --> counting sort 특성 때문에)
		// sort 결과 이용해서 재그루핑
		auto comp = [&](int u, int v) {
			if(ord[u] == ord[v]) return ord[u+t] < ord[v+t];
			return ord[u] < ord[v];
		};
		tmp[sa[0]] = 1; // 이거 꼭 필요! ord[n]이 이미 0이기 때문.
		FOR(i, 1, n) tmp[sa[i]] = tmp[sa[i-1]] + comp(sa[i-1], sa[i]);
		ord = tmp;
		if(ord[sa[n-1]] == n) break; // ord[sa[i]] = i+1이 모든 i에 대해 만족하면 정렬 완료!: 이거 유무에 따른 성능차 큼
	}
	FOR(i, 0, n) --ord[i]; // ord[sa[i]] = i를 만족한다!
	// **** GET LCP ARRAY **** //
	lcp = vi(n, 0);
	for(int i = 0, len = 0; i < n; ++i, len = max(0, len-1)) {
		if(ord[i]) {
			for(int j = sa[ord[i]-1]; s[i+len] == s[j+len]; ++len);
			lcp[ord[i]] = len; // 여기서 lcp[i] = sa상에서 i번째 suffix의 인접 lcp  임에 주의!
		}
	}
	// **** PRINT ANSWER **** //
	FOR(i, 0, n) cout << sa[i]+1 << ' '; cout << ENDL;
	cout << "x ";
	FOR(i, 1, n) cout << lcp[i] << ' '; cout << ENDL;
	return 0;
}
