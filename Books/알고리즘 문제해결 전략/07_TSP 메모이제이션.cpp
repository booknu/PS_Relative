...
map<int, double> memo[MAX][CACHE_DEPTH]; // memo[here][남은도시수][visited(key)]
...
double dp(int here, int visited) {
	...
	int remaining = n - bitcount(visited);
	double &ret = memo[here][remaining][visited];
	if (ret > 0) {
		return ret;
	}
...
void search(vector<int> &path, vector<bool> &visited, double currLength) {
	...
	// base case: remaining cities are less than CACHE_DEPTH
	if (path.size() + CACHE_DEPTH >= n) {
		...
		best = min(best, currLength + dp(path.back(), visitMask));
	}
